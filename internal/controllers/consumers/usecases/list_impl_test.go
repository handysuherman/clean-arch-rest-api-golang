...
func Test_MOCK_LIST(t *testing.T) {
	...
	mockList := []*repository.Consumer{mockArgs.repoResponse}

	testCases := []struct {
		tname         string
		body          *domain.FetchParams
		stubs         func(store *mock.MockRepository)
		checkResponse func(t *testing.T, res *domain.ConsumerList, err error)
	}{
		{
			tname: "OK",
			body:  mockArgs.listParams,
			stubs: func(store *mock.MockRepository) {
				store.EXPECT().CountList(gomock.Any(), gomock.Eq(mockArgs.countListRepoParams)).Times(1).Return(int64(1), nil)
		...
		},
		{
			...
			body:  mockArgs.listParams,
			stubs: func(store *mock.MockRepository) {
				store.EXPECT().CountList(gomock.Any(), gomock.Eq(mockArgs.countListRepoParams)).Times(1).Return(int64(0), sql.ErrConnDone)
