...
const (
	redisPrefixKey = "mtfinance:affiliated_dealers"
)

func (r *RedisRepositoryImpl) Put(ctx context.Context, key string, arg *AffiliatedDealer) {
	...
	prefixKey := helper.RedisPrefixes(
		key,
		redisPrefixKey,
		r.cfg.Databases.Redis.Prefixes.AffiliatedDealer.Prefix,
		r.cfg.Databases.Redis.AppID,
	)

	if err := r.redisClient.Set(ctx, prefixKey, payload, r.cfg.Databases.Redis.Prefixes.AffiliatedDealer.ExpirationDuration).Err(); err != nil {
		return
	}
...
func (r *RedisRepositoryImpl) Get(ctx context.Context, key string) (*AffiliatedDealer, error) {
