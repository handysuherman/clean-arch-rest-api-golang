...
func Test_MOCK_FIND_BY_ID(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			stubs: func(store *mock.MockRepository) {
				store.EXPECT().Get(gomock.Any(), gomock.Eq(mockArgs.repoResponse.ID)).Times(1).Return(mockArgs.repoResponse, nil)
		...
		},
		{
			...
			id:    mockArgs.repoResponse.ID,
			stubs: func(store *mock.MockRepository) {
				store.EXPECT().Get(gomock.Any(), gomock.Eq(mockArgs.repoResponse.ID)).Times(1).Return(nil, errors.New("redis not found"))
